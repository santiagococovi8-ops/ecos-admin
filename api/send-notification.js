export default async function handler(req, res) {
  ...
  // Sanitizar legajo igual que en la app (la barra "/" no es válida en external_id)
  const legajoId = legajo
    ? String(legajo).replace(/[^a-zA-Z0-9_\-\.@]/g, '_')
    : null;
  ...
  const target = legajoId
    ? {
        include_aliases: { external_id: [legajoId] },
        target_channel: 'push',
      }
    : {
